...
def step_2_handler(step_info, total_steps, module_path, **kwargs):
    ...
    Instructs parallel dispatch via Task tool with sequential fallback when
    the Task tool is unavailable in the sub-agent context. (ref: DL-005, RSK-002)
    """
    state_dir = kwargs.get("state_dir", "")
    return {
        "title": step_info["title"],
        "actions": [
            "READ QR items generated by decomposition:",
            f"  cat {state_dir}/qr-impl-docs.json",
            "",
            "If qr-impl-docs.json is empty or contains zero items:",
            "  No verification items were generated.",
            f"  Skip to step 4: python3 -m {module_path} --step 4 --state-dir {state_dir}",
            "",
            "Otherwise, group items by group_id.",
            "",
