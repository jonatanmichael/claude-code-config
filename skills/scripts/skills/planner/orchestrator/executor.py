...
STEPS = {
    ...
    4: {
        ...
        "pre_dispatch": [
            "<qa_integration>",
            "Before QR code review, run impl-code QA.",
            "",
            "1. DISPATCH QA decomposition:",
            "   python3 -m skills.planner.quality_reviewer.impl_code_qr_decompose --step 1 --state-dir $STATE_DIR",
            "   Context: PLAN_FILE, MODIFIED_FILES, STATE_DIR",
            "   Phase: impl-code",
            "",
            "2. Once decomposition complete, read qr-impl-code.json from STATE_DIR.",
            "",
            "3. For each item where scope != '*': dispatch parallel verifier.",
            "   For each item where scope == '*': dispatch sequential verifier.",
            "",
            "4. Aggregate results into qr-impl-code.json (update status/finding fields).",
            "",
