#!/usr/bin/env python3
"""Orchestration entry point for the impl-docs quality review phase.

Guides the quality-reviewer agent through a 4-step pipeline:
  1. Decompose  -- invoke impl_docs_qr_decompose to populate qr-impl-docs.json
  2. Fan-Out    -- dispatch parallel verify agents per group_id (ref: DL-005)
  3. Aggregate  -- tally PASS/FAIL/UNVERIFIED from qr-impl-docs.json
...
their implementation (ref: DL-003). Structure mirrors impl_code_qr.py;
only PHASE and module paths differ (ref: DL-004, DL-010).
"""
...
# PHASE must match impl_docs_qr_decompose.py and impl_docs_qr_verify.py --
...
def step_1_handler(step_info, total_steps, module_path, **kwargs):
    ...
    return {
        ...
        "actions": [
            "TASK: Run impl-docs QR decomposition to generate verification items.",
            "",
            "INVOKE the decompose sub-script and follow ALL its steps until completion:",
            f"  python3 -m skills.planner.quality_reviewer.impl_docs_qr_decompose --step 1 --state-dir {state_dir}",
            "",
...
def step_2_handler(step_info, total_steps, module_path, **kwargs):
    ...
    Loads qr-impl-docs.json at generation time, groups by group_id, and injects
    ...
    if not state_dir:
        return {
            ...
            "actions": [
                ...
                "    python3 -m skills.planner.quality_reviewer.impl_docs_qr_verify --step 1",
    ...
    qr_path = os.path.join(state_dir, "qr-impl-docs.json")
    try:
    ...
    if group_count == 0:
        return {
            "title": step_info["title"],
            "actions": [
                "READ qr-impl-docs.json -- file missing, empty, or unparseable.",
...
def step_3_handler(step_info, total_steps, module_path, **kwargs):
    ...
    return {
        "title": step_info["title"],
        "actions": [
            "RE-READ qr-impl-docs.json after all verify agents have completed:",
            f"  cat {state_dir}/qr-impl-docs.json",
            "",
...
def step_4_handler(step_info, total_steps, module_path, **kwargs):
    ...
    PASS/ISSUES keywords trigger routing at executor step 8. (ref: DL-006, DL-012)
    """
    state_dir = kwargs.get("state_dir", "")
    return {
        "title": step_info["title"],
...
if __name__ == "__main__":
    ...
    mode_main(
        ...
        extra_args=[
            # default="" preserved for backward compatibility; executor.py STEPS[7] invoke_suffix
